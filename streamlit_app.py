...
if page == "🔊 Home":
    ...
    def load_and_process_data():
        ...
        return df, vlucht_types, vluchten, route_layers, radius_layer

    # Laad de data en vluchtsoorten met behulp van de gecachte functie
    df, vlucht_types, vluchten, route_layers, radius_layer = load_and_process_data()
    ...
    # filter route layers based on selected flights.
    filtered_route_layers = []
    for layer in route_layers:
        if layer.data[0]['FlightNumber'] in selected_flights or "Alle vluchten" in selected_flights :
          filtered_route_layers.append(layer)
    ...
    # Maak een pydeck Deck (kaart)
    deck = pdk.Deck(
        layers=filtered_route_layers + [radius_layer],
        initial_view_state=initial_view_state,
        map_style="mapbox://styles/mapbox/streets-v11",
        tooltip={
            "html": "<b>Vlucht ID:</b> {FlightNumber}",
